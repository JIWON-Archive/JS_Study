...
// 타입이 function이 아니면
 ...
 this.gfn_cacheSet = function(){
     ...
     $.ajax({
         ...
         success:function(data, statusText, xhr){
             //기초코드, 설정
             libUtil.codeList = data.result.baseCodeMgtDto;
             
             //메뉴
             libUtil.menuList = data.result.menuInfoDto;
             
             //페이지 정보
             libUtil.pageList = data.result.pageSetDto;
             
             //팝업 정보
             libUtil.popList = data.result.popSetDto;
         },
         error:function(ex){
             console.log(ex);
         }
 ...
 //캐시 초기화
 this.gfn_cacheRefresh = function(){
     $.ajax({
         url: gfn_getUri('/cache/setCacheDelete'),
         type:'POST',
         async: false, // true,
         success:function(data, statusText, xhr){
             gfn_cacheSet
 ...
 //현재 페이지 그리드, 필드정보 가져오기 
 this.gfn_getCurrentPage = function(){
     var page = libUtil.pageList;
     
     var currentPage = $("#_currentMenuId").val();
     
     var pageList = libUtil.pageList.filter(function(item){return item.menuId == currentPage})
     
     var returnPage = {};
     
     for(var key in pageList){
         returnPage[pageList[key].gridId] = pageList[key];
     }
     
     return returnPage;
 }
 
 this.gfn_currentGrid_set = function(base, pageList){
 
     if(gfn_isEmpty(pageList)){
         gfn_warnBox({msg:"설정된 페이지 정보가 없습니다."});
         return false;
     }
     
     //캐시 - layoutId로 캐시 선택 
     var page = pageList[base.layoutId];
     
     //popId
     base.popId = page.gridPopId;
     
     //팝업 사이즈
     base.popWidth = page.gridPopWidth;
     base.popHeight = page.gridPopHeight;
     
     //중복가능성 기존제거
     if(gfn_isEmpty(base.layoutPosition) == false && base.layoutPosition == 'pop'){
         $("#"+base.layoutId).attr("css",page.gridStyle);
         $("#"+base.layoutId).attr("class",page.gridClass);
     }else{
         $("#"+base.layoutId).remove();
         
         $("#grid-content").append($("<div>",{id : base.layoutId, style : page.gridStyle, class : page.gridClass}));
     }
     
     
     //controll Url
     base.ctrlUrl = page.gridUrlBase;
     
     //crud 초기화
     base.crud = {};
     
     //CRUD 매칭
     //crud function 'gfn_myTab().' prefix로 붙여주기
     if(!gfn_isEmpty(page.gridUrlRead)) base.crud.read = JSON.parse(page.gridUrlRead);
     
     if(!gfn_isEmpty(page.gridUrlCreate)) {
         base.crud.create = JSON.parse(page.gridUrlCreate);
         if(!gfn_isEmpty(base.crud.create.callback)) base.crud.create.callback = base.crud.create.callback.substr(0,3) == "lfn" ? "gfn_myTab()." + base.crud.create.callback : base.crud.create.callback;
         if(!gfn_isEmpty(base.crud.create.openFunc)) base.crud.create.openFunc = base.crud.create.openFunc.substr(0,3) == "lfn" ? "gfn_myTab()." + base.crud.create.openFunc : base.crud.create.openFunc;
     }
     
     if(!gfn_isEmpty(page.gridUrlUpdate)){
         base.crud.update = JSON.parse(page.gridUrlUpdate);
         if(!gfn_isEmpty(base.crud.update.callback)) base.crud.update.callback = base.crud.update.callback.substr(0,3) == "lfn" ? "gfn_myTab()." + base.crud.update.callback : base.crud.update.callback;
     }
     
     if(!gfn_isEmpty(page.gridUrlDestroy)) {
         base.crud.destroy = JSON.parse(page.gridUrlDestroy);
         if(!gfn_isEmpty(base.crud.destroy.callback)) base.crud.destroy.callback = base.crud.destroy.callback.substr(0,3) == "lfn" ? "gfn_myTab()." + base.crud.destroy.callback : base.crud.destroy.callback;
     }
     
     //gird Title
     if(!gfn_isEmpty(page.gridTitle)){
         base.customTitle = {html : "<span style='padding : 5px 0; font-weight: bold; font-size: 20px;'>"+ page.gridTitle +"</span>"};
     }
     ...
     //팝업 html을 그려준다.
     //main div
     var $popMain = $("<div>", {id : page.gridPopId});
     //main Form
     var $popForm = $("<form>", {id : page.gridPopId + "_form", class: page.gridPopClass});
     //main Table
     var $popTable = $("<table>", {id: page.gridPopId + "_master", class: "BasicTable"});
     //colgroup
     var $popColgroup = $("<colgroup>", {id:"jtePopId"});
     //본문영역 - input box 위치
     var $popTableBody = $("<tbody>", {id : page.gridPopId + "_table_body"});
     //본문영역 - 히든 rows, data
     var $popTableHiddenRows = $("<tr>", {style : "display : none;"});
     var $popTableHiddenData = $("<td>");
     //커스텀 div 영역(그리드, 커스텀버튼)
     var $popCustomArea = $("<div>", {class:"grid-area"});
     
     //버튼영역
     var $popBtnArea = $("<div>", {class:"final-btn-area"});
     
     $popTableHiddenRows.append($popTableHiddenData);
     $popMain.append($popForm);
     $popMain.append($popCustomArea);
     $popMain.append($popBtnArea);
     $popForm.append($popTable);
     $popTable.append($popColgroup);	
     $popTable.append($popTableBody);
     
     
     //null인경우 예외처리
     page.gridPopCol = gfn_isNull(page.gridPopCol) == true ? "" : page.gridPopCol;  
     
     //그리드정보에 설정된 col 계산
     var popCols = page.gridPopCol.split(","); 
     
     //col 갯수
     const colCnt = popCols.length;
     
     //form h
     var curCnt = 0;
     
     //설정된 값 만큼 콜그룹에 append
     for(value of popCols){
         $popColgroup.append('<col style="width:'+value+'%;">');
     }
 
     //필드설정  데이터 최초 팝업 순서로 정렬 - 그리드는 object 형태로 return 하기 때문에 처리 후 정렬
     
     page.pageFieldList.sort(function(a, b) {return gfn_nvl(JSON.parse(a.fieldPop).fieldPopNum,999999) - gfn_nvl(JSON.parse(b.fieldPop).fieldPopNum,999999)});
     
     //검색된 필드정보 loop
     for(var key in page.pageFieldList){
         var field = page.pageFieldList[key];
         
         //grid - model 세ㅔ팅
         if(field.fieldType == "text"){
             base.model.fields[field.fieldId] = {type: 'string'};
         }else if(field.fieldType == "textarea"){
             base.model.fields[field.fieldId] = {type: 'string'};
         }else if(field.fieldType == "selBox"){
             base.model.fields[field.fieldId] = {type: 'string'};
         }else if(field.fieldType == "currency"){
             base.model.fields[field.fieldId] = {type: 'int'};
         }else if(field.fieldType == "numeric"){
             base.model.fields[field.fieldId] = {type: 'int'};
         }else if(field.fieldType == "datepicker"){
             base.model.fields[field.fieldId] = {type: 'date'};
         }else if(field.fieldType == "datetimepicker"){
             base.model.fields[field.fieldId] = {type: 'date'};
         }
         
         //필드네임 - 다국어가 있을 수 있어 예외처리해준다.
         var fieldName = '';
         if (!gfn_isNull(field.fieldName)){
             //다국어 예외처리
             try{
                 fieldName = eval(field.fieldName);
             }catch(e){
                 fieldName = field.fieldName;	
             }
         }
         //필드 정보 각각 담기
         var fieldGrid = JSON.parse(field.fieldGrid);
         var fieldPop = JSON.parse(field.fieldPop);
         var fieldSearch = JSON.parse(field.fieldSearch);
         var fieldValidation = JSON.parse(field.fieldValidation);
         var fieldExcel = JSON.parse(field.fieldExcel);
         
         //--그리드 컬럼 세팅--
         var columns = {};
         if(!gfn_isEmpty(fieldGrid) && !gfn_isEmpty(fieldGrid.fieldGridYn) && fieldGrid.fieldGridYn == 'comnTrue'){
             //버튼
             if(field.fieldType == 'fieldTypeButton'){
                 columns = {field : field.fieldId, width : fieldGrid.fieldGridWidth};
                 //그리드 커스텀(버튼)
                 if(!gfn_isNull(fieldGrid.fieldGridCustom)){
                     var btnCustom = JSON.parse(fieldGrid.fieldGridCustom);
                     var btmTemplate = '';
                     for(key in btnCustom){
                         var clickEvent = btnCustom[key].click.replace(/lfn\_/gi,'gfn_myTab\(\)\.lfn\_');
                         var msg = btnCustom[key].msg.indexOf("gfn_") == 0 ? eval(btnCustom[key].msg) : btnCustom[key].msg;
                         btmTemplate += '<button class="k-button k-primary" type="button" onclick = "' + clickEvent + '">' + msg + '</button>';
                     }
                     columns.template = btmTemplate;
                 }
             }
             //selectBox
             else if(field.fieldType == 'fieldTypeCheck'){
                 columns = {selectable : true, width : fieldGrid.fieldGridWidth};
             }
             //그외
             else{
                 columns = {field : field.fieldId, width : fieldGrid.fieldGridWidth};
                 //그리드 커스텀
                 if (!gfn_isNull(fieldGrid.fieldGridCustom)){
                     columns.template = fieldGrid.fieldGridCustom.replace(/lfn\_/gi,'gfn_myTab\(\)\.lfn\_');
                 }
             }
             
             //그리드 - 타이틀
             if (!gfn_isNull(field.fieldName)){
                 columns.title = fieldName;
             }
             
             
             //그리드 - 헤더 속성(스타일, 클래스)
             if (!gfn_isNull(fieldGrid.fieldGridHeaderAttr)){
                 try{
                     columns.headerAttributes = JSON.parse(fieldGrid.fieldGridHeaderAttr);
                 }catch(e){
             ...
             //그리드 - 속성(스타일, 클래스)
             if (!gfn_isNull(fieldGrid.filedGridAttributes)){
                 try{
                     columns.attributes = JSON.parse(fieldGrid.filedGridAttributes);
                 }catch(e){
                     columns.attributes = "";
                 }
             }
             ...
             //그리드 정렬 순서
             if (!gfn_isNull(fieldGrid.fieldGridNum)){
                 columns.fieldGridNum = fieldGrid.fieldGridNum;
             }
             
             //그리드 포맷
             if (!gfn_isNull(fieldGrid.fieldGridFormat)){
                 columns.format = fieldGrid.fieldGridFormat;
             }
             
             
             base.columns.push(columns);
         }
         //--그리드 컬럼세팅 종료--
         ...
         //--그리드 팝업 세팅--
         if(!gfn_isEmpty(fieldPop) && !gfn_isEmpty(fieldPop.fieldPopYn) && fieldPop.fieldPopYn == 'comnTrue'){
             if(!gfn_isEmpty(fieldPop.fieldPopHidden) && fieldPop.fieldPopHidden == 'comnFalse'){
                 
                 var $popTableRows;
                 if(curCnt == 0) $popTableRows = $("<tr>");
                 
                 if(field.fieldType != 'fieldTypePopHtml' && field.fieldType != 'fieldTypePopGrid'){
                     $popTableRows.append($("<td>").append($("<i>",{class:"fas fa-caret-right"})).append(fieldName+" : "));
                 }
                 
                 var popForamt = {};
                 
                 try{
                     popForamt = JSON.parse(fieldPop.fieldPopFormat);
                 }catch(e){
                     popForamt = fieldPop.fieldPopFormat;
                 }
                 
                 if(!gfn_isEmpty(popForamt) && typeof(popForamt) == "object"){
                     var $popInput = null;
                     var $popButton = null;
                     var $popHidden = null;
                     if(popForamt.type == 'selBox'){
                         $popInput = $("<input>",Object.assign(popForamt,{format : gfn_getCode(field.fieldType).cdVal, name : field.fieldId, notmod : fieldPop.fieldPopReadOnly}));
                         $popTableRows.append($("<td>",{colspan : fieldPop.fieldPopCol-1}).append($popInput));
                     }else if(popForamt.type == 'msg'){
                         $popInput = $("<input>",{class: "btn-input", type:gfn_getCode(field.fieldType).cdVal, format : gfn_getCode(field.fieldType).cdVal, name : field.fieldId});
                         $popButton = $('<button>', Object.assign(popForamt,{class : "k-button k-primary jte-msgBox", type : "button", html : gfn_getMsg("pop_msgLang","다국어")}));
                         $popHidden = $("<input>", {type : "hidden", id : "msg_" + popForamt.defId});
                         $popTableRows.append($("<td>",{colspan : fieldPop.fieldPopCol-1}).append($popInput).append($popButton).append($popHidden));
                     }else if(popForamt.type == 'button'){
                         //버튼이름 다국어 예외처리
                         var btnName = '';
                         if (!gfn_isNull(popForamt.btnMsg)){
                             try{
                                 btnName = eval(popForamt.btnMsg);
                             }catch(e){
                                 btnName = popForamt.btnMsg;	
                             }
                         }
                         popForamt.onclick = popForamt.onclick.replace(/lfn\_/gi,'gfn_myTab\(\)\.lfn\_');
                         $popInput = $("<input>",{class: "btn-input", type : gfn_getCode(field.fieldType).cdVal, format : gfn_getCode(field.fieldType).cdVal, name : field.fieldId, readonly : true});
                         $popButton = $('<button>', Object.assign(popForamt, {class : "k-button k-primary", type : "button", onclick : popForamt.onclick, html : btnName}));
                         $popTableRows.append($("<td>",{colspan : fieldPop.fieldPopCol-1}).append($popInput).append($popButton));
                     }else if(field.fieldType == 'fieldTypeTextarea'){
                         $popInput = $("<textarea>", Object.assign(popForamt, {name : field.fieldId, notmod : fieldPop.fieldPopReadOnly}));
                         $popTableRows.append($("<td>",{colspan : fieldPop.fieldPopCol-1}).append($popInput));
                     }else if(field.fieldType == 'fieldTypeFile'){
                         $popInput = $("<input>", Object.assign(popForamt, {name : field.fieldId, type : "file", format : "file", notmod : fieldPop.fieldPopReadOnly}));
                         $popHidden = $("<input>", {type : "hidden", id : "insertAttach", value: "[]"});
                         $popHidden += $("<input>", {type : "hidden", id : "deleteAttach", value: "[]"})
                         $popTableRows.append($("<td>",{colspan : fieldPop.fieldPopCol-1}).append($popInput).append($popHidden));
                     }else{
                         $popInput = $("<input>", Object.assign(popForamt, {type:gfn_getCode(field.fieldType).cdVal, format : gfn_getCode(field.fieldType).cdVal, name : field.fieldId, notmod : fieldPop.fieldPopReadOnly}));
                         $popTableRows.append($("<td>",{colspan : fieldPop.fieldPopCol-1}).append($popInput));
                     }
                 }else if(field.fieldType == 'fieldTypePopHtml'){
                     popForamt = popForamt.replace(/lfn\_/gi,'gfn_myTab\(\)\.lfn\_');
                     $popTableRows.append($("<td>",{class: 'cache-grid-html', colspan : fieldPop.fieldPopCol}).append(popForamt));
                 }else if(field.fieldType == 'fieldTypePopGrid'){
                     $popCustomArea.append(popForamt);
                 }else{
                     $popInput = $("<input>", {type:gfn_getCode(field.fieldType).cdVal, format : gfn_getCode(field.fieldType).cdVal, name : field.fieldId, notmod : fieldPop.fieldPopReadOnly});
                     $popTableRows.append($("<td>",{colspan : fieldPop.fieldPopCol-1}).append($popInput));
                 }
                 curCnt += fieldPop.fieldPopCol;
                 
                 if(colCnt == curCnt){
                     $popTableBody.append($popTableRows);
                     curCnt = 0;
                 }
             }else{
                 //히든값 영역
                 $popInput = $("<input>", {type : "hidden", name : field.fieldId});
                 $popTableHiddenData.append($popInput);
             }
         }
         
         //유효성검사
         if(!gfn_isEmpty(fieldValidation) && !gfn_isEmpty(fieldValidation.fieldValidationYn) && fieldValidation.fieldValidationYn == 'comnTrue'){
             base.validation[field.fieldId] = {
                 messages : fieldValidation.fieldValidationMsg,
                 fieldId : field.fieldId,
                 check : fieldValidation.fieldValidationCheck
             }
         }
         
         //검색박스
         if(!gfn_isEmpty(fieldSearch) && !gfn_isEmpty(fieldSearch.fieldSearchYn) && fieldSearch.fieldSearchYn == 'comnTrue'){
             
             if(gfn_isEmpty(fieldSearch.fieldSearchTrueValue))fieldSearch.fieldSearchTrueValue = field.fieldDb;
             
             base.selectBox.push({text : fieldName, value : fieldSearch.fieldSearchTrueValue, searchNum : fieldSearch.fieldSearchNum});
         }
         
         
         //엑셀유무
         if(!gfn_isEmpty(fieldExcel)){
             
             fieldExcel.fieldExcelImport = gfn_isEmpty(fieldExcel.fieldExcelImport) == true ? false : fieldExcel.fieldExcelImport;
             
             fieldExcel.fieldExcelExport = gfn_isEmpty(fieldExcel.fieldExcelExport) == true ? false : fieldExcel.fieldExcelExport;
             
             base.excel.push({field : field.fieldId, text : fieldName, import : fieldExcel.fieldExcelImport , export : fieldExcel.fieldExcelExport})
         }
     }
     
     //팝업 히든영역에 숨김처리 필드들 append 
     $popTableBody.append($popTableHiddenRows);
     
     //팝업 버튼 처리
     $popBtnArea
     .append($("<button>", {class:"k-button k-primary jte-create final-btn", type:"button", style : "display:none;",html : gfn_getMsg("pop_save","저장")}))
     .append($("<button>", {class:"k-button k-primary jte-update final-btn", type:"button", style : "display:none;",html : gfn_getMsg("pop_save","저장")}))
     .append($("<button>", {class:"k-button k-primary jte-destroy final-btn", type:"button", style : "display:none;",html : gfn_getMsg("pop_delete","삭제")}))
     .append($("<button>", {class:"k-button k-primary final-btn", type : "button", onclick : "gfn_closePop('"+ page.gridPopId +"')" ,html : gfn_getMsg("pop_cancel","취소")}));
 
 
     //만들어진 form html popup 영역에 append
     if($("#"+base.popId).length == 0){
         $("#pop-content").append($popMain);
     }
     
     //이거 다듬기!
     $("#" + page.gridPopId + "_form .cache-grid-html").find(".cache-html-msg").each(function(){
         ...
         $(this).html(msg + $(this).html());
     })
     
     //컬럼소트
     base.columns.sort(function(a, b) {return gfn_nvl(a.fieldGridNum ,999999) - gfn_nvl(b.fieldGridNum,999999)});
     
     //검색소트
     base.columns.sort(function(a, b) {return gfn_nvl(a.searchNum ,999999) - gfn_nvl(b.searchNum,999999)});
 ...
 this.gfn_getUri = function(bizUri){
         ...
         return (document.location.pathname + "/" + bizUri)
                 ...
                 .replace(/[^:\/\/]\/\//g, function(arg){return arg.replace("/","")})
                 .replace(/[^:\/\/]\/\//g, function(arg){return arg.replace("/","")})
                 .replace(":///", "://").replace("//", "/").replace("/admin/", "/").replace("/login/", "/").replace("/main.do/", "/").replace("/main/", "/").replace("/home.do/", "/").replace("/home/", "/").replace("/tabMain.do/", "/").replace("/tabMain/", "/");  // biz화면 기본URL
 };
 ...
 //cache 도입으로 인한 신규 코드리스트 호출 - 2021-03-08 jkkim
 this.gfn_getCode = function(cdId){
     var rtnCode;
     
     var tempCode = libUtil.codeList.find(function(element){if(element.cdId == cdId) return true})
     
     if(gfn_isEmpty(tempCode)){
         rtnCode = {cdId :null, cdVal:null, cdNm:null}
     }else{
         if(gfn_isEmpty(tempCode.baseCodeList)){
             rtnCode = tempCode;
         }else{
             rtnCode = tempCode.baseCodeList;
             rtnCode.sort(function(a, b) {return a.sortNum - b.sortNum;});
         }
     }
     return gfn_objCopy(rtnCode);
 }
 
 //기존 기초코드 사용X 2021-03-08 jkkim
 ...
 //		}
 //		
 //		$.ajax({async: false, url: this.gfn_getUri('/common/code/get'), data:{code:grpId}
 //			, error:function(data){
 //				console.warn(grpId+"그룹의 코드정보 요청이 실패해도 에러는 표시하지 않는다.\n"+JSON.stringify(data));
 ...
 this.gfn_getMsg = this.gfn_getComMsg = function(){
     ...
     try {
         ...
         if(gfn_isEmpty(result)){
             ...
             $.ajax({
                     async:false, 
                     url: "/common/msg/getComMsg", 
                     data:{code:key, /*params:params,*/ def:params[1]},
                     success:function(data){
     ...
     result = gfn_isNull(result) == true ? '' : result;
 ...
 /** 자바스크립트단에서 로그아웃하고 로그인창으로 이동 **/
 this.gfn_logout = function(prmt){
     var logoutUrl = '/logout';
     //세션만료로 인한 로그아웃 처리
     if(!gfn_isNull(prmt) && prmt == "session"){
         logoutUrl += '?session=out';
     }
     
     $.ajax({
         url: "/form/devtol/userConn/setUserConn",
         type:'POST',
         data: JSON.stringify({userId : $("#_loginUserId").val(), connId : $("#_loginSessionId").val(),action : "U"}),
         traditional :true,
         contentType : 'application/json',
         dataType : 'json',
         success:function(data){
             location.href = gfn_getUri(logoutUrl);
         },error:function(ex){
             console.log(ex);
         }
     });
     ...
     options.callback = function(){
         ...
         $("#t_passwordYN").change(function(e){
             if($("#t_passwordYN").is(":checked")){
                 ...
                 gfn_resizePop('jteConfigForm',{width : 450, height:396}); /*원래 510, 290*/
             }else{
                 $(".jteConfigFomr_pwZone").hide();
                 gfn_resizePop('jteConfigForm',{width : 450, height:270}); /*원래 350, 230*/
             }
         });
     }
 ...
 //비밀번호 변경 팝업
 this.gfn_pwChangePop = function(){
     var options = {
         modal:true, 
         width: "420px", 
         height: "240px",
         id:"jtePwChangeForm", 
         title:"비밀번호 변경(3개월 초과)", 
         workCd:"", 
         bdSeq:"",
         resizable:true,
         animation:{open :{effects:"expand:vertical fadeIn"},
     ...
     setCookie( hideCookieUser , "todayShow", 1);
     
     var monthCookieUser = $("#_loginUserId").val()+ "Month";
     setCookie( monthCookieUser , "monthShow", 1);
     
     $(document).ready(function() { 
         $("#p_pwToday").on('click', function() { //하루 동안 열지 않기 클릭시
             if ( $(this).prop('checked') ) {
                 setCookie(hideCookieUser, "todayHide", 1);
                 gfn_closePop('jtePwChangeForm');
             } 
         });
         
         $("#p_pwMonth").on('click', function() { //3개월 후 변경 클릭시
             if ( $(this).prop('checked') ) {
                 setCookie(monthCookieUser, "monthHide", 90);
                 gfn_closePop('jtePwChangeForm');
             } 
         });
     });
 ...
 this.gfn_myConfigSave = function(formID){
     ...
     if($("#t_passwordYN").is(":checked") || formID == "jtePwChangeForm_form"){
         ...
         if(!pwdPtn.test(formObj.password)){
             ...
             if(formID == "jtePwChangeForm_form") {
                 $("#p_password").focus();
             } else {
                 $("#t_password").focus();
             }
             ...
             gfn_warnBox({msg:gfn_getMsg("pop_pwMismatch")});
             if(formID == "jtePwChangeForm_form") {
                 $("#p_password").focus();
             } else {
                 $("#t_password").focus();
     ...
     $.ajax({
         ...
         success: function(data){
             ...
             gfn_loading(false);
             gfn_errBox({msg : ex.responseJSON.message});			//오류가 발생했습니다.
         }
     });
 }
 ...
 this.gfn_gridResize = function() {
     ...
     if($('#grid-content').length !=0) {
         $("#grid-content").height($(window).height() - ($("#grid-content").prev().offset().top + $("#grid-content").prev().outerHeight(true)));
     }
 ...
 //메뉴영역 높이 조정
 this.gfn_menuResize = function() {
     //좌측메뉴 높이 지정
     $(".k-drawer-container").height($(window).height() - ($(".my-config-area").outerHeight(true)+ $("#toolbar").outerHeight(true)));
     
     $("#panelbar > ul").height($("#panelbar").outerHeight(true) - $("#menuFilterArea").outerHeight(true)-2);
 }
 ...
 this.gfn_serializeObject = function(param){
     ...
     for(key in rtnObj){
         if(rtnObj[key] == "" && $("#" + param + " [name="+ key +"]").attr("type") == "hidden"){
 ...
 //ajax로 리스트를 불러온다.
 this.gfn_ajaxCallByList = function(type, prmt){
     var typeObj = {};
     
     switch(type){
         case "place" :	//위치정보
             typeObj.url = "/form/basMgt/operMgt/rlehoMgt/getPlaceListAll";
         break;
         case "user" : //사용자정보
             typeObj.url = "/form/basMgt/userMgt/userInfo/getUserList";
         break
         case "svcMenu" : //메뉴정보
             typeObj.url = "/form/basMgt/menuMgt/menuInfo/getMenuTree";
         break
         case "itemMgt" : //개별 품목
             typeObj.url = "/form/item/itemStockMgt/getItemStockList";
         break
         case "bomMtrl" : //bom자재
             typeObj.url = "/cform/basMgt/bom/bomMgt/getBomList";
         break
         case "tableInfo" : //테이블 필드 정보
             typeObj.url = "/excelUtil/selectTableInfo";
         break
     }
